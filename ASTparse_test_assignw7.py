...
noneed = ["detailed","SCOPE","TOPSCOPE","SHIFTL","IF",]
need1 = ["VAR"]
need2 = ["VARREF"]
stop = ["CSTMT"]   ####################简化AST
need3 = ["VAR","VARREF","AND","OR","XOR","ADD","NOT"]
need4 = ["AND","OR","XOR","ADD","NOT","COND"]
...
fname = r'Treefiles/Vb14.txt'#---------------------------------------------------------------------------------------------------改文件名字
fresult = r'Process/Vb14_1.txt'#---------------------------------------------------------------------------------------------------改文件名字
...
detaset = glob.glob('Process/Vb14_1.txt')#---------------------------------------------------------------------------------------------------改文件名字
Node = []
stopp = 0
need5 = ["VAR","DOT"]
for deta in detaset:
    ...
    with open(deta) as d:
        for line in d:
            if isInArray(need5, line):
                ffwidth = line.split()[5]
                result_ffw.append(ffwidth)

            else:
                result_ffw.append(0)
        result_ffw = np.array(result_ffw)
        result_ffw = result_width[2:]

    with open(deta) as d:
        for line in d:
            if isInArray(need4, line):
                cname = line.split()[1]
                if "VAR" not in cname:
                    com_name.append(cname)
                    line2 = line.replace('/', ' ')
                    line3 = line2.replace(')', '')
                    cwidth1 = line3.split()[-1]
                    cwidth = cwidth1.replace('w', '')
                    cwidth = cwidth.replace('s', '')
                    com_width.append(cwidth)
            if isInArray(stop, line):
                break
            if stopp == 3:
                break
            elif isInArray(stop2, line):
                stopp = stopp+1



        com_name = np.array(com_name)
        com_width = np.array(com_width)
...
print(result_varref)
print(result_width)
print("-------------------------------FF WIDTH CHECK-------------------------------")
print(result_ffw)


print("-----------------------------COM CELLS CHECK----------------------")
print("Name")
print(com_name)
print("Width")
print(com_width)
...
ncom = 0
lxx = 0
for lxx in range(0, len(com_width)):
    ncom += int(com_width[lxx])
...
f = open('Process/Vb14_re.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
while w < loop:
    print('$$$$$$$$$$$$$$$$$$')
    #a = 'Node{}'.format(w)
    #print(a.const)
...
#f = open('create.txt',mode='{}.txt'.format(result_name[0]))   tracing003
fl = open('Process/Vb14_f.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
while wl < loop:
    ...
    if result_name[wl] == 'CONST':
        ...
        fl.write('\n')
        wl = wl + 1
    if result_name[wl] == 'ASSIGNDLY':
        reline = '{} {}'.format(vars()['Node{}'.format(wl)].name, 1)
        fl.writelines(reline)
        # f.write(reline)
        fl.write('\n')
        wl = wl + 1
...
flname = r'Process/Vb14_f.txt'#---------------------------------------------------------------------------------------------------改文件名字
flresult = r'Process/Vb14_feature.txt'#---------------------------------------------------------------------------------------------------改文件名字
...
detaset2 = glob.glob('Process/Vb14_feature.txt')#---------------------------------------------------------------------------------------------------改文件名字
...
while k71 < k1 :
    if k71== k1:
        break
    if name2[k71] == "CONST" or name2[k71]=="MUX":
...
while kl < len(result_name):
    if "OUTPUT" in linecache.getline('Process/Vb14_1.txt', kl) and len(result_edge[kl]) == 2:#-------------------------------------------------------------改文件名字
...
while lxx < len(result_name):
    pzz = linecache.getline('Treefiles/Vb14.txt', lxx)
    pzz = pzz.replace('w', ' ')
    pzz = pzz.replace(')', ' ')
    VARR = pzz.split()[-1]
    NamE = pzz.split()[0]
    ...
    if len(NamE) == 4 and VARR == "VAR":#-------------------------------------------------------------改文件名字

        FFRwidth.append(width)
        lxx = lxx + 1
    else:
        lxx = lxx + 1
...
for lxx in range(0, len(FFRwidth)):
    ttw += int(FFRwidth[lxx])
print("Real TOTAL FF WIDTH BITS")
print(ttw)
...
while klc < len(result_name):
    if "CFUNC" in linecache.getline('Process/Vb14_1.txt', klc):#-------------------------------------------------------------改文件名字
...
while kla < len(result_name):
    if "ASSIGNDLY" in linecache.getline('Process/Vb14_1.txt', kla):#-------------------------------------------------------------改文件名字
...
while klw < len(result_name):
    if "ASSIGNW" in linecache.getline('Process/Vb14_1.txt', klw):#-------------------------------------------------------------改文件名字
...
#f = open('create.txt',mode='{}.txt'.format(result_name[0]))   tracing003
fl = open('Process/Vb14_T.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
...
while k33 < k1:
    if "Flip-Flop" in linecache.getline('Process/Vb14_T.txt', i):#---------------------------------------------------------------------------------------------------改文件名字
        FFposition.append(i)
        i = i + 1
        k33 = k33 +1
        ff = ff +1

    if "ASSIGNW" in linecache.getline('Process/Vb14_T.txt', i):#---------------------------------------------------------------------------------------------------改文件名字
        FFposition.append(i)
        i = i + 1
        k33 = k33 +1
...
DFposition = []
while k331 < k1:
    if "ASSIGNW" in linecache.getline('Process/Vb14_T.txt', i1):#---------------------------------------------------------------------------------------------------改文件名字
        AAposition.append(i1)
        i1 = i1 + 1
        k331 = k331 +1

    if "Flip-Flop" in linecache.getline('Process/Vb14_T.txt', i1):#---------------------------------------------------------------------------------------------------改文件名字
...
result = glob.glob('Process/Vb14_TFF.txt')#---------------------------------------------------------------------------------------------------改文件名字
Node = []
...
while ii < ij:
    ...
    elif com_name[ii] == "NOT":
        notw = int(com_width[ii]) * 0.532
        AST_width2.append(notw)
        ii = ii +1


    elif com_name[ii] == "OR":
        orw = int(com_width[ii]) * 1.064
        AST_width2.append(orw)
        ii = ii + 1

    elif com_name[ii] == "AND":

        andw = int(com_width[ii]) * 1.064

        AST_width2.append(andw)
        ii = ii + 1

    elif com_name[ii] == "XOR":
        xorw = int(com_width[ii]) * 1.596

        AST_width2.append(xorw)
        ii = ii + 1


    elif com_name[ii] == "XNOR":
        xnorw = int(com_width[ii]) * 1.596

        AST_width2.append(xnorw)
        ii = ii + 1

    elif com_name[ii] == "COND":
        muxw = int(com_width[ii]) * 1.862
        AST_width2.append(muxw)
        ii = ii + 1

    elif com_name[ii] == "ADD":
        addw = int(com_width[ii]) * 1.862
        AST_width2.append(addw)
        ii = ii + 1
...
astresult = r'results/Area/Vb14_AST.txt'
with open(astresult, 'w', encoding='UTF-8') as astr:
    ...
    seqn = 0
    comn = 0
    while iii < len(FFRwidth):
        ast = '{} {} {}'.format("FLIP-FLOP",4.522 * int(FFRwidth[iii]),"um2")
        astr.writelines(ast)
        astr.write('\n')
        iii = iii +1
    iii = 0
    while iii < len(AST_width2):
        ast = '{} {} {}'.format(com_name[iii],AST_width2[iii],"um2")
        astr.writelines(ast)
        astr.write('\n')
        iii = iii +1
    ...
    dffwidth = 4.522 * ttw
    tttwidth = dffwidth + comwidth
    sqwidthbit = '{} {} {} {} {}'.format("Sequential area are", dffwidth, "um2","Numbers are ",ttw)
    cmwidthbit = '{} {} {} {} {}'.format("Combinational area are", comwidth, "um2", "Numbers are ",ncom)
    ttwidthbit = '{} {} {}'.format("Total area are",tttwidth, "um2")
    time_end = time.time()
    time_c = time_end - time_start
    tttime = '{} {} {}'.format(" AST parser cost",time_c,"seconds")
    ASTver = '{} '.format(" AST parser version is ASTparser_test_assignw 7")
