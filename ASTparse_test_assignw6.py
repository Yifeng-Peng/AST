...
fname = r'Treefiles/Vs344.txt'#---------------------------------------------------------------------------------------------------改文件名字
fresult = r'Process/Vs344_1.txt'#---------------------------------------------------------------------------------------------------改文件名字
...
detaset = glob.glob('Process/Vs344_1.txt')#---------------------------------------------------------------------------------------------------改文件名字
Node = []
...
f = open('Process/Vs344_re.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
while w < loop:
    print('$$$$$$$$$$$$$$$$$$')
    #a = 'Node{}'.format(w)
    #print(a.const)
...
#f = open('create.txt',mode='{}.txt'.format(result_name[0]))   tracing003
fl = open('Process/Vs344_f.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
while wl < loop:
...
flname = r'Process/Vs344_f.txt'#---------------------------------------------------------------------------------------------------改文件名字
flresult = r'Process/Vs344_feature.txt'#---------------------------------------------------------------------------------------------------改文件名字
...
detaset2 = glob.glob('Process/Vs344_feature.txt')#---------------------------------------------------------------------------------------------------改文件名字
...
while kl < len(result_name):
    if "OUTPUT" in linecache.getline('Process/Vs344_1.txt', kl) and len(result_edge[kl]) == 2:#-------------------------------------------------------------改文件名字
...
while klc < len(result_name):
    if "CFUNC" in linecache.getline('Process/Vs344_1.txt', klc):#-------------------------------------------------------------改文件名字
...
while kla < len(result_name):
    if "ASSIGNDLY" in linecache.getline('Process/Vs344_1.txt', kla):#-------------------------------------------------------------改文件名字
...
while klw < len(result_name):
    if "ASSIGNW" in linecache.getline('Process/Vs344_1.txt', klw):#-------------------------------------------------------------改文件名字
...
#f = open('create.txt',mode='{}.txt'.format(result_name[0]))   tracing003
fl = open('Process/Vs344_T.txt', mode='w')#---------------------------------------------------------------------------------------------------改文件名字
...
while k33 < k1:
    if "Flip-Flop" in linecache.getline('Process/Vs344_T.txt', i):#---------------------------------------------------------------------------------------------------改文件名字
        FFposition.append(i)
        i = i + 1
        k33 = k33 +1
        ff = ff +1

    if "ASSIGNW" in linecache.getline('Process/Vs344_T.txt', i):#---------------------------------------------------------------------------------------------------改文件名字
        FFposition.append(i)
        i = i + 1
        k33 = k33 +1
...
DFposition = []
while k331 < k1:
    if "ASSIGNW" in linecache.getline('Process/Vs344_T.txt', i1):#---------------------------------------------------------------------------------------------------改文件名字
        AAposition.append(i1)
        i1 = i1 + 1
        k331 = k331 +1

    if "Flip-Flop" in linecache.getline('Process/Vs344_T.txt', i1):#---------------------------------------------------------------------------------------------------改文件名字
...
if ff == 0:
    shutil.copy("Process/Vs344_T.txt", "Process/Vs344_TFF.txt")# ---------------------------------------------------------------------------------------------------改文件名字
    print("NO FF IN THE DESIGN")
else:
    if len(FFposition) == 0:
        print("NO FF IN THE DESIGN")
    if len(FFposition) == 1:
        count = len(open("Process/Vs344_T.txt").readlines())  # ---------------------------------------------------------------------------------------------------改文件名字
        flname = r'Process/Vs344_T.txt'  # ---------------------------------------------------------------------------------------------------改文件名字
        flresult = r'Process/Vs344_TFF.txt'  # ---------------------------------------------------------------------------------------------------改文件名字
        # open(fname, 'r', encoding='gb2312')
        GET2 = ["Flip-Flop"]
        ...
        with open(flname, 'r', encoding='UTF-8') as fll:
            with open(flresult, 'w', encoding='UTF-8') as kl:
                ...
                a = FFposition[0] + 1
                print(a)
                print(linecache.getline('Process/Vs344_T.txt', a))#---------------------------------------------------------------------------------------------------改文件名字
                while a < FFposition[-1]:
                    aaa= '{}'.format(linecache.getline('Process/Vs344_T.txt', a))#---------------------------------------------------------------------------------------------------改文件名字
                    kl.writelines(aaa)
                    a = a + 1
                print("END")
    else:
        ...
        with open(flname, 'r', encoding='UTF-8') as fll:
            with open(flresult, 'w', encoding='UTF-8') as kl:
                ...
                while cc1 < AAposition[-1]:                  # 这里应该是用CFUNC来计算，到底在哪里停止，FF可以先不打开计算
                    if "ASSIGNW" in linecache.getline('Process/Vs344_T.txt', cc1) :
                        cc1 = cc1 + 1
                        EDD = test*(EDD+1)
                    ...
                    if "Flip-Flop" in linecache.getline('Process/Vs344_T.txt', cc1) :
                        ccc = '{}'.format(linecache.getline('Process/Vs344_T.txt', cc1))
                        kl.writelines(ccc)
                        # if a < len(DFposition):
                        #     cc1 = DFposition[a]
                        #     a = a + 1
...
result = glob.glob('Process/Vs344_TFF.txt')#---------------------------------------------------------------------------------------------------改文件名字
Node = []
...
astresult = r'results/Area/Vs344_AST.txt'
with open(astresult, 'w', encoding='UTF-8') as astr:
    ...
    ASTver = '{} '.format(" AST parser version is ASTparser_test_assignw 6，DLY打开，Assignw的个数还是在DLY结束后的CFUNC2那里,第一个ASSIGNW从第一个ASSIGNDLY后开始计算")
...
dot.render('{}.gv'.format('results/tree/Vs344_ast'), view=True)#---------------------------------------------------------------------------------------------------改文件名字
